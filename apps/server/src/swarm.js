...
export async function runSwarm(taskId) {
  ...
  if (genesToSave.length) {
    ...
    for (const g of genesToSave) {
      ...
      // 同时沉淀为 experience 记忆
      dbb.memories.unshift({
        id: uid('mem-'),
        type: 'experience',
        content: g.claim,
        summary: g.claim.slice(0,40),
        importance: (g.gdi||70)/100,
        confidence: 0.85,
        source: 'swarm',
  ...
  // 失败记忆
  const failed = task.swarm.subtasks.filter(s=>s.status==='failed');
  if (failed.length) {
    try {
      const { createMemory } = await import('./memory/index.js');
      createMemory({ type: 'failure', content: `蜂群任务 ${task.title} 有 ${failed.length} 个子任务失败：${failed.map(f=>f.title).join(', ')}，需复盘`, summary: `失败：${failed.length}子任务`, importance: 0.7, confidence: 0.9, source: 'swarm', projectId: task.workspaceId, tags: ['失败','蜂群'] });
    } catch {}
  }
